...
describe("TrackConverterV2", () => {
  ...
  describe("replaceTracksInPlaylist()", () => {
    ...
    it("If tracks replaced, returns number of tracks and playlists updated", async () => {
      ...
      converter.addTracksToPlaylist = () => true;

      const response = await converter.replaceTracksInPlaylist({
        tracks: { href: 0, total: 0 },
      });
      const expected = [2, 1];
  ...
  describe("replaceUserSavedTracks()", () => {
    ...
    it("Returns number of tracks and playlists updated", async () => {
      ...
      const response = await converter.replaceUserSavedTracks({
        tracks: { href: 0, total: 0 },
      });
      const expected = [2, 0];
      expect(response).toEqual(expected);
  ...
  describe("findOldAlbums()", () => {
    it("Returns albumsToReplace, albumsToAdd, totalTracksToReplace", async () => {
      ...
      const response = await converter.findOldAlbums([
        { album: { id: "1OYARuagDrpgNNQ4loO1Cs", total_tracks: 33 } },
      ]);
      const expected = [
        ["1OYARuagDrpgNNQ4loO1Cs"],
        ["5YqltLsjdqFtvqE7Nrysvs"],
        33,
      ];
      expect(response).toEqual(expected);
    });
  ...
  describe("findOldTracks()", () => {
    it("Returns tracksToReplace and tracksToAdd", async () => {
      ...
      const response = await converter.findOldTracks([
        { track: { id: "1OYARuagDrpgNNQ4loO1Cs" } },
      ]);
      const expected = [
        [
          {
  ...
  describe("saveUserAlbums()", () => {
    ...
    it("If not albumIds or if not albumIds.length return false", async () => {
      const converter = new TrackConverterV2("", false);
  ...
  describe("removeUserSavedAlbums()", () => {
    ...
    it("If not albumIds or if not albumIds.length return []", async () => {
      const converter = new TrackConverterV2("", false);
  ...
  describe("deleteTracksInPlaylist()", () => {
    ...
    it("If not albumIds or if not albumIds.length return []", async () => {
      ...
      const response = await converter.deleteTracksInPlaylist(
        "reindeerMan",
        88,
        []
      );
  ...
  describe("addTracksToPlaylist()", () => {
    ...
    it("If not albumIds or if not albumIds.length return false", async () => {
      ...
      const response = await converter.addTracksToPlaylist(
        "reindeerMan",
        88,
        []
      );
  ...
  describe("fetchCurrentUserID()", () => {
    beforeAll(() => {
      jest
        .spyOn(axios, "get")
        .mockImplementation(() => Promise.resolve({ data: { id: 5 } }));
    });
  ...
  describe("fetchUserPlaylists()", () => {
    beforeAll(() => {
      jest.spyOn(axios, "get").mockImplementation(() =>
        Promise.resolve({
          data: { items: ["playlist1", "playlist2"], total: 4 },
        })
      );
    });
    ...
    it("Calls axios.get", async () => {
      ...
      expect(await converter.fetchUserPlaylists("reindeerMan")).toEqual(
        expectedValue
      );
    });
  });
